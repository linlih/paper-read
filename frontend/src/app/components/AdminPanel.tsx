...
function paperUploader(paper: Paper) {
  return paper.uploadedBy || paper.uploaded_by;
}
function annotationPaperID(annotation: Annotation) {
  return annotation.paperId || annotation.paper_id || '';
}
function annotationUserID(annotation: Annotation) {
  return annotation.userId || annotation.author_id || '';
}
function annotationText(annotation: Annotation) {
  return annotation.selectedText || annotation.targets?.[0]?.quote_exact || '';
}
...
export function AdminPanel({ papers, users, annotations, currentUser, onBack, onUpdatePaper, onDeletePaper, onDeleteUser, t }: AdminPanelProps) {
  const [adminPapers, setAdminPapers] = useState<Paper[]>(papers);
  const [adminUsers, setAdminUsers] = useState<UserType[]>(users);
  const [adminAnnotations, setAdminAnnotations] = useState<Annotation[]>(annotations);
  ...
  useEffect(() => {
    api<{ papers: Paper[] }>('/api/admin/papers')
      .then(payload => setAdminPapers(payload.papers))
      .catch(() => {});
    api<{ users: UserType[] }>('/api/admin/users')
      .then(payload => setAdminUsers(payload.users))
      .catch(() => {});
    api<{ annotations: Annotation[]; targets?: Array<{ annotation_id?: string; quote_exact?: string }> }>('/api/admin/annotations')
      .then(payload => {
        const targetsByAnnotation = new Map<string, Array<{ annotation_id?: string; quote_exact?: string }>>();
        for (const target of payload.targets || []) {
          const id = target.annotation_id || '';
          targetsByAnnotation.set(id, [...(targetsByAnnotation.get(id) || []), target]);
        }
        setAdminAnnotations(payload.annotations.map(annotation => ({
          ...annotation,
          paperId: annotation.paperId || annotation.paper_id || '',
          userId: annotation.userId || annotation.author_id || '',
          selectedText: annotation.selectedText || targetsByAnnotation.get(annotation.id)?.[0]?.quote_exact || '',
  ...
  async function saveEdit() {
    ...
    const payload = await api<{ paper: Paper }>(`/api/papers/${editingPaper.id}`, {
      method: 'PATCH',
      body: JSON.stringify({ title: editTitle, abstract: editAbstract }),
    });
    const updated = { ...payload.paper, htmlContent: editHtml };
    setAdminPapers(prev => prev.map(paper => paper.id === updated.id ? updated : paper));
    onUpdatePaper(updated);
    setEditingPaper(null);
  }

  async function deletePaper(id: string) {
    await api<{ ok: true }>(`/api/papers/${id}`, { method: 'DELETE' });
    setAdminPapers(prev => prev.filter(paper => paper.id !== id));
    setAdminAnnotations(prev => prev.filter(annotation => annotationPaperID(annotation) !== id));
    onDeletePaper(id);
  }

  async function deleteUser(id: string) {
    await api<{ ok: true }>(`/api/admin/users/${id}`, { method: 'DELETE' });
    setAdminUsers(prev => prev.filter(user => user.id !== id));
    onDeleteUser(id);
  }

  const tabs: { id: Tab; label: string; icon: React.ReactNode; count: number }[] = [
    { id: 'papers', label: t.papersTab, icon: <FileText size={15} />, count: adminPapers.length },
    { id: 'users', label: t.usersTab, icon: <Users size={15} />, count: adminUsers.length },
    { id: 'annotations', label: t.annotationsTab, icon: <Edit3 size={15} />, count: adminAnnotations.length },
  ];
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded ${paper.status === 'published' || paper.status === 'ready' ? 'bg-green-100 text-green-700' : paper.status === 'error' || paper.status === 'blocked' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      ...
                      <p className="text-[#7A7165] text-xs">{paperAuthors(paper).join(', ')}</p>
                      <div className="flex items-center gap-3 mt-2 text-xs text-[#7A7165]">
                        <span className="flex items-center gap-1"><Clock size={10} />{formatDate(paperDate(paper))}</span>
                        <span className="flex items-center gap-1"><User size={10} />{adminUsers.find(u => u.id === paperUploader(paper))?.name || '—'}</span>
                        <span>{adminAnnotations.filter(a => annotationPaperID(a) === paper.id).length} {t.annotationsTab.toLowerCase()}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => { if (window.confirm(t.confirmDeletePaper)) deletePaper(paper.id); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-red-500 hover:bg-red-50 transition-colors text-xs">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#7A7165] mt-0.5">
                      {adminAnnotations.filter(a => annotationUserID(a) === user.id).length} {t.annotationsTab.toLowerCase()} · {adminPapers.filter(p => paperUploader(p) === user.id).length} {t.papersTab.toLowerCase()}
                    </div>
                  </div>
                </div>
                {user.id !== currentUser.id && user.role !== 'admin' && (
                  <button onClick={() => { if (window.confirm(t.confirmDeleteUser)) deleteUser(user.id); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-red-500 hover:bg-red-50 transition-colors text-xs">
          ...
          <div className="flex flex-col gap-3">
            ...
            {adminAnnotations.map(ann => (
              <div key={ann.id} className="bg-[#FDFAF6] rounded-xl border border-[#1E1C1A]/8 p-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-[#1E1C1A] italic mb-1.5 line-clamp-2">"{annotationText(ann)}"</p>
                  <div className="flex items-center gap-3 text-xs text-[#7A7165] flex-wrap">
                    <span>{adminPapers.find(p => p.id === annotationPaperID(ann))?.title?.slice(0, 35)}…</span>
                    <span>·</span>
                    <span>{adminUsers.find(u => u.id === annotationUserID(ann))?.name}</span>
                    <span>·</span>
